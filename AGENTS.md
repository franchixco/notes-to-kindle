...
- Per send, the following leaves the machine: note title, configured author, the full note content (including expanded embedded-note text up to depth 3), approved local JPEG/opaque PNG/static GIF image bytes, and the generated EPUB. Original image bytes can retain EXIF, GPS, thumbnails, camera details, and color profiles; the plugin confirms image count and total size before sending.
...
- Capabilities (keep accurate): embedded Markdown notes expand up to depth 3; validated vault-local JPEG, opaque PNG, and static GIF images are included after an explicit preflight; remote and unsupported images are never fetched and render as alt text or omission markers; raw HTML is escaped; only safe link schemes (`http:`, `https:`, `mailto:`) survive.
